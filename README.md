...
Linux/macOS: `./gradlew build`；`./gradlew runClient`。
可选 Create 开发环境：`./gradlew build -Pwith_create=true`。
首次下载需要访问 Gradle、NeoForge、Mojang 与 Maven。完整检查：`./gradlew coreCheck`。
